...
def feature_matching_temp (descriptor1,descriptor2,method):
    ...
    for kp1 in range(keyPoints1):
        ...
        # matched_image = cv2.drawMatches(original_img, kp1, template_img, kp2,matched_features[:100], template_img, flags=2)
        # plt.imshow(matched_image)
        # plt.imsave("images/output.png")
    return matched_features

# # Apply feature matching using SSD ::
# kp1, descriptor1 = SIFT.computeKeypointsAndDescriptors(original_img,save_img=0)
# kp2, descriptor2 = SIFT.computeKeypointsAndDescriptors(template_img,save_img=0)
# matched_features = feature_matching_temp(descriptor1, descriptor2,"SSD")
# matched_features = sorted(matched_features, key=lambda x: x.distance, reverse=True)
# matched_image = cv2.drawMatches(original_img, kp1, template_img, kp2,matched_features[:20], template_img, flags=2)
...
# feature_matching_temp(descriptor1,descriptor2, "SSD", original_img,template_img)
# plt.show()
